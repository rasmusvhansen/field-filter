...
export function App() {
  ...
  const [areaIds, setAreaIds] = useState<string[]>(['a1', 'a2', 'a3']);
  const [fieldIds, setFieldIds] = useState<string[]>(['f1', 'f2', 'f3']);
  ...
  const removeField = (id: string) => {
    ...
    setFieldIds(fieldIds.filter((i) => i !== id));
  };
  const removeArea = (id: string) => {
    setAreas(without(areas, id));
    setAreaIds(areaIds.filter((i) => i !== id));
  };
  useEffect(() => {
    getMapUrl(Object.values(areas).concat(Object.values(fields))).then((url) => setImg(url));
  }, [areas, fields]);
  return (
    <>
      <ExcelUpload
      ...
      <div class="area-wrapper">
        <div>
          ...
          {fieldIds.map((id) => (
            <Polygon
              key={id}
              area={fields[id] || { id, coords: [], color: '#0000ff' }}
              onUpload={(area) => setFields({ ...areas, [area.id]: area })}
        ...
        <div class="areas">
          ...
          {areaIds.map((id) => (
            <Polygon
              key={id}
              area={areas[id] || { id, coords: [], color: '#0000ff' }}
